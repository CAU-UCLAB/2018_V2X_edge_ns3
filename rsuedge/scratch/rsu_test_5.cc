...
NS_LOG_COMPONENT_DEFINE("RsuTest_v5");
...
int main (int argc, char *argv[])
{
    ...
    WifiMacHelper rsuMac;
    WifiMacHelper vehMac;
    WifiHelper wifi;
    ...
    rsuMac.SetType("ns3::ApWifiMac", "Ssid", SsidValue(ssid));
    vehMac.SetType("ns3::StaWifiMac", "Ssid", SsidValue(ssid));

    
    //5. Create NetDevices
    NetDeviceContainer rsuDevice;
    rsuDevice = wifi.Install(phy, rsuMac, rsuNodes);

    NetDeviceContainer vehicleDevice;
    vehicleDevice = wifi.Install(phy, vehMac, vehicleNodes);
    ...
    rsuApp.Start(Seconds(1));
    rsuApp.Stop(Seconds(50.0));
    ...
    rsuVehiApp.Start(Seconds(0));
    rsuVehiApp.Stop(Seconds(50.0));
    ...
    phy.EnablePcap ("rsu_test_5_rsuDevice", rsuDevice);
    //phy.EnablePcap ("rsu_test_5_vehicleDevice", vehicleDevice);
